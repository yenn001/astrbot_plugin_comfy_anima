...
Prompt Lab 接受固定 Seed、基础分层、可选资产池和锁定层，每批生成 1–6 个可复现候选。候选只是短期草稿：
...
4. 要实际出图，应把确认结果交给普通 QQ 绘图入口；该入口仍会执行既有权限、敏感词、LoRA 提交前复核与工作流校验，Prompt Lab 不提供旁路。
...
- 显式添加 `--llm`（短写 `--l`）时，输入会先经过当前绘图导演，转换为“有序 Tags + 一句英文场景描述”的混合提示词；`--raw` / `--no-llm` 可明确保持原样。
