...
因此，WebUI 的“当前生图工作流”只出现三个可选项是正常行为。独立 RTX、Quick 和 LanPaint 会显示在“检查六管线依赖”结果中，但不会进入生图工作流下拉框，也不会被 `/comfy_use` 设为默认文生图入口。`/改图` 是复用图片反推和当前 Anima 生图管线的编排能力，不额外增加第七份 ComfyUI 工作流。
...
- 无蒙版整图改图：引用一张图后直接说换衣、换背景、换表情或重新画一张；插件按保守、平衡或自由模式重新生成整张图。
...
这是“理解原图后重新生成”，不是像素级图生图，因此不能保证原姿势或背景完全不变。当前版本故意不把 `--denoise` 伪装成改图强度；真正的整图 img2img 将作为独立工作流另行接入。
...
普通“画某角色穿新衣”属于文生图；引用现有图片后的普通换衣、换背景、换表情或重新画一张会进入 `/改图`。只有明确提到遮罩、蒙版、白色 / 透明区域、局部区域或 inpaint 时，才会进入局部重绘。
...
这是预期行为。下拉框只选择 `base`、`rtx`、`iterative` 三条文生图管线。独立 RTX 用 `/放大`，Quick / LanPaint 用 `/重绘`，无蒙版整图语义重绘用 `/改图`。`/改图` 复用反推和当前 Anima 管线，因此不会成为第四个可选文生图工作流。点击“检查六管线依赖”可以查看六份 ComfyUI 工作流。
...
`/改图` 会反推原图事实后重新调用 Anima 生成，因此只保证语义层面的保留约束，不承诺像素、姿势或背景完全一致。需要只改局部时请提供遮罩并使用 `/重绘`；需要更强构图保持时，应等待或接入专用整图 img2img / ControlNet 工作流。
