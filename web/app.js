...
function applyTheme(name, {persist = true} = {}) {
  ...
  if (persist) {
    try {
      window.localStorage.setItem("comfy-anima-theme", theme);
    } catch (_error) {
      // A private browser can reject storage while still allowing theme switching.
    }
  }
}

function initializeThemePicker() {
  applyTheme(document.documentElement.dataset.theme || "workshop", {persist: false});
...
async function saveSettings(event) {
  ...
  try {
    ...
    if (data.reload_scheduled) {
      setTimeout(() => window.location.replace("/login"), 2600);
    }
  } catch (error) {
    showToast(error.message, true);
  } finally {
...
async function saveConfigProfile() {
  ...
  if (exists && !window.confirm(`配置档案“${name}”已存在，确定用当前设置覆盖吗？`)) return;
  setBusy(button, true, "正在保存…");
  try {
    const data = await api("/api/config-profiles", {
      method: "POST",
...
async function deleteConfigProfile() {
  ...
  if (!window.confirm(`确定删除配置档案“${name}”吗？当前插件设置不会被删除。`)) return;
...
async function deletePreset(identifier) {
  if (!window.confirm(`确定删除组合“${identifier}”吗？`)) return;
  try {
...
async function cancelSelectedTask() {
  ...
  if (!window.confirm(`确定取消任务 ${selectedTask.run_id} 吗？`)) return;
...
async function clearConsoleLogs() {
  if (!window.confirm("清空本插件专属持久日志视图？AstrBot 文件日志和任务事件不会被删除。")) return;
...
function initializeArchivePreferences() {
  ...
  auto.checked = localStorage.getItem("comfy-anima-auto-archive") === "true";
  fetchFirst.checked = localStorage.getItem("comfy-anima-archive-fetch-first") !== "false";
  auto.addEventListener("change", () => localStorage.setItem("comfy-anima-auto-archive", String(auto.checked)));
  fetchFirst.addEventListener("change", () => localStorage.setItem("comfy-anima-archive-fetch-first", String(fetchFirst.checked)));
}
...
document.querySelector("#metadata-all").addEventListener("click", (event) => {
  if (window.confirm("确定调用 LoRA Manager 为全库获取 Civitai 元数据吗？该操作可能需要较长时间。")) {
    fetchLoraMetadata([], {button: event.currentTarget});
  }
});
...
document.querySelector("#archive-all").addEventListener("click", (event) => {
  if (window.confirm("确定让绘图导演为当前全部 LoRA 重新执行 AI 建档吗？这可能产生多次模型调用。")) {
    runLoraArchive("all", {button: event.currentTarget});
  }
});
